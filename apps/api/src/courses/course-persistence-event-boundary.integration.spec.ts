...
describe('Course persistence event boundary integration', () => {
  ...
  it('preserves pending domain events when a Course is persisted', async () => {
    const { moduleRef, repository } = await createTestingModule();
    ...
    const eventsAfterSave = course.getDomainEvents();

    expect(eventsAfterSave).toHaveLength(1);

    expect(eventsAfterSave[0]?.eventId).toBe(eventsBeforeSave[0]?.eventId);

    expect(eventsAfterSave[0]?.eventName).toBe(eventsBeforeSave[0]?.eventName);

    expect(eventsAfterSave[0]?.aggregateId).toBe(course.id.toString());

    await moduleRef.close();
  });

  it('does not consume domain events when the Course is read back', async () => {
    const { moduleRef, repository } = await createTestingModule();
    ...
    if (stored === null) {
      throw new Error('Expected the persisted Course to be found.');
    }

    expect(stored).toBe(course);

    const events = stored.getDomainEvents();
  ...
  it('keeps the event collection unchanged across repeated persistence operations', async () => {
    const { moduleRef, repository } = await createTestingModule();
  ...
  it('keeps domain events available after application-level persistence', async () => {
