...
describe('CourseVersionRepository contract', () => {
  it('accepts a persistence implementation matching the contract', async () => {
    ...
    expect(await repository.findAllByCourseId(courseId)).toEqual([]);

    expect(await repository.findLatestByCourseId(courseId)).toBeNull();

    expect(await repository.findPublishedByCourseId(courseId)).toBeNull();

    expect(await repository.existsByCourseIdAndVersion(courseId, 1)).toBe(
      false,
    );
