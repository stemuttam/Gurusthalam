...
const courseVersionRollbackActorTypes =
  Object.values(
    COURSE_VERSION_AUDIT_ACTOR_TYPE,
  ) as [
...
const sourceVersionIdSchema =
  nonEmptyStringSchema;

const rollbackReasonSchema =
  nonEmptyStringSchema.max(
    500,
...
const actorTypeSchema =
  z.enum(
    courseVersionRollbackActorTypes,
  );

const actorIdSchema =
  nonEmptyStringSchema;

export const courseVersionRollbackInputSchema =
  z
    .object({
      courseId:
        courseIdSchema,
...
export type CourseVersionRollbackInputSchema =
  z.infer<
