...
} from '../contracts/course-version-rollback.validation.js';

export class DefaultCourseVersionRollbackApplicationService
  implements CourseVersionRollbackApplicationService
{
  constructor(
  ...
  async rollback(
    ...
    return this.persistence.execute(
      async (
        ...
        const source =
          await transaction.findVersionById(
            sourceVersionId,
          );
            ...
            [
              {
                ...
                message:
                  'The rollback source belongs to a different Course.',
              },
            ],
          );
        }
        ...
        const latest =
          await transaction.findLatestVersionByCourseId(
            courseId,
          );
        ...
        const audit =
          CourseVersionAudit.create({
            ...
            metadata: {
              sourceVersionId:
                rollback.lineage.sourceVersionId.value,

              sourceVersion:
                rollback.lineage.sourceVersion,

              targetVersionId:
                rollback.lineage.targetVersionId.value,

              targetVersion:
                rollback.lineage.targetVersion,

              relation:
                rollback.lineage.relation,
            },
          });
        ...
        await transaction.appendLineage(
          rollback.lineage,
        );
